    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 审核回答申诉
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            ...
            >
                ...
                <el-table-column prop="qa_content" label="申诉理由" width="160">
                    <template slot-scope="scope">
                        <el-popover
                            placement="top-start"
                            width="200"
                            trigger="hover">
                            <div>{{scope.row.qa_content}}</div>
                            <div class="qContent" slot="reference">{{scope.row.qa_content}}</div>
                        </el-popover>
                    </template>
                </el-table-column>
                <el-table-column prop="qa_time" label="申诉时间" width="160"></el-table-column>
                <el-table-column label="审核操作" width="160" align="center">
                    <template slot-scope="scope">
                        <div class="checkAction">
                            ...
                            >通过申诉</el-button>
                            <disappeal-answer :q_id="scope.row.qa_id" v-on:getData='getData'></disappeal-answer>
                        </div>
                    </template>
                </el-table-column>
            </el-table>
...
import disappealAnswer from './module/disappealAnswer.vue'
export default {
    name: 'basetable',
    components:{
        disappealAnswer
    ...
    methods: {
        ...
        getData() {
            ...
            this.$axios({
                method: "post",
                url: 'administrator/getappealreply',
        ...
        handleAgree(index, row) {
            this.$confirm('确定解除屏蔽回答？', '提示', {
                ...
                .then(() => {
                    ...
                    this.$axios({
                        method: "post",
                        url: 'administrator/agreeappealreply',
                        data: this.$Qs.stringify(data)
                    })
                    .then(function(res) {
