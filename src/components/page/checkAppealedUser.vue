    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 审核用户申诉
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            ...
            >
                ...
                <el-table-column prop="ua_content" label="申诉理由"></el-table-column>
                <el-table-column prop="ua_time" label="申诉时间"></el-table-column>
                <el-table-column label="审核操作" width="180" align="center">
                    <template slot-scope="scope">
                        <div class="checkAction">
                            ...
                            >通过申诉</el-button>
                            <disappeal-user :q_id="scope.row.ua_id" v-on:getData='getData'></disappeal-user>
                        </div>
                    </template>
                </el-table-column>
            </el-table>
...
import disappealUser from './module/disappealUser.vue'
export default {
    components:{
        disappealUser
    },
    ...
    methods: {
        ...
        getData() {
            ...
            this.$axios({
                method: "post",
                url: 'administrator/getappealuser',
                data: this.$Qs.stringify(data)
            })
            .then(function(res) {
                console.log("申诉用户列表",res);
        ...
        handleAgree(index, row) {
            this.$confirm('确定解除拉黑用户？', '提示', {
                ...
                .then(() => {
                    ...
                    this.$axios({
                        method: "post",
                        url: 'administrator/agreeappealUser',
                        data: this.$Qs.stringify(data)
                    })
                    .then(function(res) {
        ...
        handleDisagree(index, row) {
            ...
            let data = {
                token: window.sessionStorage.getItem('token'),
                ua_id:row.ua_id,
                ua_reason:this.ua_reason
            }
            let _this=this
            // console.log(data)
            this.$axios({
                method: "post",
                url: 'administrator/disagreeappealuser',
                data: this.$Qs.stringify(data)
            })
            .then(function(res) {
        ...
        agreeAllSelection() {
            ...
            let _this=this
            if(ids==''){
                _this.$message.error('请先选择要解除拉黑的用户')
                return
            }
            // console.log(data)
            this.$confirm('确定解除拉黑用户？', '提示', {
